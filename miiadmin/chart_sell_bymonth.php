...
<head>
	...
	<title>MiiStore | Statistika Penjualan per Bulan</title>
			...
			<ul class="sidebar-nav">
				...
				<li class="sidebar-child"><a href="#"><i class="fa fa-th"></i> Grafik<i class="sidebar-fa fa fa-angle-down pull-right"></i></a>
					<ul class="sidebar-second-child" style="display:block;">
						...
						<li class="active"><a href="chart_sell_bymonth.php">Penjualan per Bulan</a></li>
				...
				<div class="row">
					<div class="col-lg-12" style="margin-bottom: 8%;">
						...
						<div class="chartjs">
							...
							$query = "SELECT i1.month, COALESCE(i2.amount,0) AS totalamount
										  FROM
										  ...
										  LEFT JOIN (SELECT DATE_FORMAT(creation_date,'%m') AS month, 
										  SUM(price -(price * discount / 100)) AS amount 
										  FROM items 
										  WHERE YEAR(creation_date) = YEAR(CURRENT_DATE)
										  GROUP BY DATE_FORMAT(creation_date,'%m')) i2 ON i2.month = i1.month 
										  GROUP BY i1.month DESC";
								...
								$(document).ready(function() {
									...
									var myChart = new Chart(ctx, {
										...
										options: {
											title: {
												display: true,
												text: 'Statistika Total Penjualan per Bulan'
											},
